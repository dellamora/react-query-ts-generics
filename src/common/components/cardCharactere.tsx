...
const CardCharactere = ({ rickandmortyAPI }: Props): JSX.Element => {
  return (
    <div className="grid-cols-charactersCards gap-8 space-y-6 lg:grid lg:w-full lg:space-y-0 lg:p-20">
      {rickandmortyAPI.map(character => {
        return (
          <div
            key={`character-${character.id}`}
            className="h-full overflow-hidden rounded-md border border-secondary bg-secondary shadow-md"
          >
